...
fn en(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "set.times" => "Start – end (HH:MM)",
        "set.proxy" => "Proxy URL",
        "set.repo" => "Update repo (owner/name)",
        "set.ytdlp" => "yt-dlp binary",
        ...
        "n.saved" => "saved",
        "n.sched_bad" => "use HH:MM, 24h (e.g. 01:00)",
        "row.progress" => "{p}% of {t} • {s}/s • ETA {e}",
        "row.progress_na" => "{b} • {s}/s",
        "app.footer" => "{n} rows • {a} active • {d} / {t} • {s}/s",
        // CLI messages.
